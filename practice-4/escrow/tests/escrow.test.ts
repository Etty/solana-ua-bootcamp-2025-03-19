...
describe("escrow", () => {
  ...
  test("Offer closed by Alice, Alice received back tokens and SOL, Bob can't accept this offer", async () => {
    const offeredUsdc = new BN(10_000_000);
    const wantedWif = new BN(100_000_000);
    const getTokenBalance = getTokenBalanceOn(connection);
    const aliceSolBeforeOffer = await connection.getBalance(alice.publicKey);
    ...
    const aliceSolAfterOffer = await connection.getBalance(alice.publicKey);
    // Check if comission was refunded
    expect(aliceSolAfterOffer - aliceSolBeforeOffer).toEqual(0);
    const accountInfo = await connection.getAccountInfo(offerAddress);
    // check if offer closed
    expect(accountInfo).toEqual(null);

    // Check if Alice received back her tokens
    expect(await getTokenBalance(aliceUsdcAccount)).toEqual(new BN(90_000_000));

    try {
      await takeOfferTx(offerAddress, bob);
    } catch (err) {
      // Check if Bob couldn't take closed offer, and his tokens preserved
      expect(await getTokenBalance(bobWifAccount)).toEqual(new BN(200_000_000));
    }
  });
});
